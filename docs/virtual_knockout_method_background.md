...
当前模型不是全基因组 KO 大模型，也不是把每个基因的敲除结果都记下来。它真正学习的是：

```text
某些真实 KO 的系统先验特征
和
这些 KO 引起的 pathway / protein / ATAC 状态变化方向
之间的映射关系
```
...
```text
KO gene prior vector -> KO state delta
```

### 5.1 已接入的 KO 数据覆盖

当前原型已经接入和整理过多类 perturbation 数据：
...
合并看，当前接入过的是百级别扰动基因覆盖，大约 129 个基因名。这个数字表示“本地实验接入过的数据覆盖范围”，不表示某一个模型已经一次性训练了全部 129 个基因。

### 5.2 为什么没学过的基因也可以预测

对一个训练集中没见过的基因，模型不会凭空猜。它会先构建这个基因的系统先验表示：
...
```text
某类 prior pattern 通常导致某类 state delta
```

那么它就可以把这种规律外推到未见基因：

```text
unseen gene prior vector
-> predicted KO delta
-> control cell + predicted KO delta
-> virtual KO cell
```

这就是我们方法能做 unseen gene virtual knockout 的原因：它利用的是基因在系统网络中的位置，而不是只依赖 one-hot gene label。

### 5.3 这种外推的边界

未见基因预测更可信的情况：

- 目标基因在 Reactome/MSigDB/TF-target/PPI/motif 先验中有良好覆盖。
- 目标基因和训练过的 KO 基因位于相似通路或网络模块。
- 当前细胞类型中该基因或相关 pathway 有表达/活性。
- 多模态数据给出一致信号，例如 RNA、protein、ATAC 同时支持同一方向。

未见基因预测风险较高的情况：

- 目标基因没有真实 KO 训练样本。
- 目标基因几乎不在系统先验网络中。
- 当前细胞类型中该基因表达很低或调控机制未知。
- KO 效应主要来自现有先验没有覆盖的机制。
- 双敲组合中两个基因都没见过，且缺少相似组合可参考。

所以，未见基因预测应该被解释为：

```text
基于系统先验的 zero-shot / few-shot 外推，
适合候选筛选和方向判断，
但需要置信度标记，不能当作已验证真实 KO。
```
...
| 未见基因需要置信度标记 | 可以做系统先验外推，但不是所有 unseen gene 都可靠 |
