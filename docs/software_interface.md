...
如果输入里包含真正的 peak-level ATAC 特征，尤其是很多 0/非 0 的稀疏 peak，推荐进一步尝试：

```powershell
--shape-calibrate quantile
```

`quantile` 会从训练 KO 学习 peak 的开放比例和分位数形状，而不只是方差。它更适合回答：

```text
敲除后，这个 peak 是在更多细胞里打开，还是在更少细胞里打开？
打开的那部分细胞里，accessibility 的分布形状是否接近真实 KO？
```

注意：`quantile` 主要改善单细胞分布形状，所以 AUC、direction cosine、MAE 可能保持不变；真正应关注的是 `distribution improvement`、`improved fraction` 和 peak-level violin/heatmap 图。

如果输入是原始非负 ATAC peak/count 矩阵，`quantile` 可以启用更强的 open/closed 形状约束。如果输入已经是中心化或标准化后的 peak state score，软件会自动关闭 hard-zero，只保留分位数形状校准，并在 `calibration.csv` 里记录 `control_open_fraction`、`target_open_fraction` 和 `hard_zero_shape_feature`，方便检查。
